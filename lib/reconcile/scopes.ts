...
// Explains why AdScale's headline spend/ROAS differs from a FILTERED Meta Ads Manager view. AdScale reports
...
// not a mystery. Same source for every scope, so the comparison is apples-to-apples within AdScale.
...
// The scopes, from broadest (what AdScale shows) to narrowest (what a filtered Meta view usually shows).
export function computeScopes(ads: ReconAd[]): ReconReport {
  return {
    total: ads.length,
    scopes: [
      tally("whole", "Whole account", "Every ad that spent in the window, all statuses, catalog included. This is AdScale's default headline number.", ads, () => true),
