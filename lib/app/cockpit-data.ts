...
export type CockpitData =
  | { connected: true; view: CockpitView; metrics: AccountMetrics; accountName: string; accountId: string; dateParam: string; adsAnalyzed: number; processed: ProcessedCounts; days: number; userEmail?: string }
...
export async function loadCockpit(days: number): Promise<CockpitData> {
  ...
  if (live.status === "connected" && live.adsAnalyzed > 0) {
    return { connected: true, view: live.view, metrics: live.metrics, accountName: live.accountName, accountId: live.accountExternalId, dateParam, adsAnalyzed: live.adsAnalyzed, processed: live.processed, days: effectiveDays, userEmail };
  }
