...
// Cockpit types live in ./types (Phase 8 extraction); imported here for the assembler logic and re-exported
// so existing `@/lib/cockpit/analyze` type imports keep working.
import type { CockpitAdInput, Priority, CockpitAction, CockpitAd, CreativeHalfLife, SpendContributor, CockpitView } from "./types.ts";
export type { CockpitAdInput, Priority, CockpitAction, CockpitAd, CreativeHalfLife, SpendContributor, CockpitView } from "./types.ts";
// Action mapping + queue ordering live in ./actions (Phase 8 extraction); imported for the assembler,
// orderByMoneyAtStake re-exported so its test keeps importing it from here.
import { DECISION_VERDICT, DECISION_LABEL, orderByMoneyAtStake, actionFor } from "./actions.ts";
export { orderByMoneyAtStake } from "./actions.ts";
