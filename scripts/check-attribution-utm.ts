...
import { utmLink, tagAdScaleLinks } from "../lib/growth/attribution.ts";
...
const tagged = tagAdScaleLinks("Try https://adscaledigital.co/app for this.", { source: "hackernews", content: "t1" });
...
const twice = tagAdScaleLinks(tagged, { source: "hackernews", content: "t1" });
ok((twice.match(/utm_source=/g) || []).length === 1, "already-tagged link is not tagged again");

// non-AdScale links are untouched
const other = tagAdScaleLinks("See https://example.com/x and https://reddit.com/r/PPC", { source: "reddit" });
