      ...
      {rows.map((a) => (
        <li key={a.id} className="rounded-[10px] border border-[var(--hairline)] bg-[var(--bg)] p-4">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <button type="button" disabled={busy === a.id} onClick={() => act(a.id, "published")} className="rounded-lg bg-[var(--ink)] px-3 py-1.5 text-[12px] font-medium text-white disabled:opacity-50">Publish to /blog</button>
            <button type="button" disabled={busy === a.id} onClick={() => act(a.id, "archived")} className="rounded-lg border border-[var(--hairline)] px-3 py-1.5 text-[12px] text-[var(--ink-muted)] hover:text-[var(--ink)] disabled:opacity-50">Dismiss</button>
          </div>
        </li>
      ))}
    </ul>
