...
export function EventSwitcher() {
  ...
  const summary = sel.size === 0 ? "All events" : `${sel.size} event${sel.size === 1 ? "" : "s"}`;
  const list = (options ?? []).filter((o) => humanize(o).toLowerCase().includes(query.trim().toLowerCase()));

  return (
    <FilterPopover label="Event" summary={summary} pending={pending} dialogLabel="Filter by optimization event">
      <Input
        value={query}
