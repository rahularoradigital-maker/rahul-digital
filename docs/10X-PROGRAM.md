...
| 5 | **Instant app** — background precompute + materialized rollups | **data-layer (b6)** _(doc said "25"; f3 assigned it to b6 directly — reconcile if 25 is real)_ | 🟡 slice 1 DONE: `account_rollups` (migration 0035, applied live) + `lib/rollups/{account,pure}.ts` compute-on-sync (reuses `computeScopes`, identical math) + reconcile reads it with self-heal fallback + populate hooks on ingest/run & cron/sync (on res.complete). `check:rollups` in check:all. Verified: table ready, whole-account 90d = the reconcile "whole" number. Next: creative + health rollups; wire cockpit headline. |
